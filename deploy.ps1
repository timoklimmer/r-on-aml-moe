...
# -- load configuration
Write-Host "▶️▶️ Loading configuration..." -ForegroundColor DarkCyan
...
Write-Host
if (!((Read-Host -Prompt "Do you want to continue with the shown configuration? [y/n]") -match "[yY]")) { 
    exit 1
}
Write-Host

# -- compute several required variable values
Write-Host "▶️▶️ Computing several required variable values..." -ForegroundColor DarkCyan
...
Write-Host "▶️▶️ Build the local docker image..." -ForegroundColor DarkCyan
...
Write-Host "▶️▶️ Starting the local docker image..." -ForegroundColor DarkCyan
...
Write-Host "▶️▶️ Live" -ForegroundColor DarkCyan
curl "http://localhost:8000/live" --noproxy localhost
Write-Host "`n`n▶️▶️ Ready" -ForegroundColor DarkCyan
curl "http://localhost:8000/ready" --noproxy localhost
Write-Host "`n`n▶️▶️ Score" -ForegroundColor DarkCyan
...
Write-Host "▶️▶️ Login to Azure..." -ForegroundColor DarkCyan
if (![string]::IsNullOrWhiteSpace($config.AZURE_SUBSCRIPTION_ID)) {
    az account set -s $($config.AZURE_SUBSCRIPTION_ID)
}

# -- deploy model(s)
Write-Host "▶️▶️ Deploying model..." -ForegroundColor DarkCyan
...
Write-Host "`n▶️▶️ Pushing container image to Azure Container Registry..." -ForegroundColor DarkCyan
az acr login --name $acr_name
docker push $image_tag

# -- deploy endpoint to AML managed online endpoint
Write-Host "`n▶️▶️ Creating/updating AML Managed Online Endpoint..." -ForegroundColor DarkCyan
...
Write-Host "`n▶️▶️ Getting scoring URI and authentication keys for webservice..." -ForegroundColor DarkCyan
...
Write-Host "`n▶️▶️ Test webservice in Azure..." -ForegroundColor DarkCyan
